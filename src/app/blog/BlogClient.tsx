...
export default function BlogClient({ posts, categories, basePath = '/health-topics' }: BlogClientProps) {
 ...
 const matchesSearch = searchQuery === '' || post.title.toLowerCase().includes(searchQuery.toLowerCase()) || (post.excerpt && post.excerpt.toLowerCase().includes(searchQuery.toLowerCase()))
 ...
 className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${ activeCategory === 'All' ? 'bg-warm-900 text-white shadow-md' : 'bg-warm-100 text-warm-600 hover:bg-warm-200' }`}
 >
 All Posts
 </button>
 {categories.map((cat) => (
 ...
 className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${ activeCategory === cat ? 'bg-warm-900 text-white shadow-md' : 'bg-warm-100 text-warm-600 hover:bg-warm-200' }`}
 >
 {cat}
 </button>
 ))}
 ...
 return (
 <motion.div key={post._id || slug} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
