        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
          ...
          >
            ...
            <h1 className="text-4xl lg:text-6xl font-display font-bold text-warm-900 mb-6 leading-[1.1]">
              ...
              <span className="text-gradient-primary">healthiest</span> life.
            </h1>
            
            <p className="text-xl text-warm-500 leading-relaxed mb-10">
              Expert insights, practical tips, and real stories to help you 
            ...
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 rounded-2xl border border-warm-200 bg-white/80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-primary-200 focus:border-primary-400 transition-all text-warm-700 placeholder:text-warm-400"
              />
            </div>
          </motion.div>
        </div>
            ...
            >
              <Link href={`/blog/${featuredPost.slug}`} className="group block">
                <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                  <div className="aspect-[4/3] rounded-3xl bg-gradient-to-br from-primary-100 via-warm-100 to-sage-100 overflow-hidden">
                  ...
                  <div>
                    ...
                    <h2 className="text-3xl lg:text-4xl font-display font-bold text-warm-900 mb-4 group-hover:text-primary-700 transition-colors leading-tight">
                      {featuredPost.title}
                    </h2>
                    ...
                    <div className="flex items-center gap-6 text-sm text-warm-400">
                      <span className="flex items-center gap-2">
                        <User size={14} />
                        {featuredPost.author || 'Aliento Medical'}
                      </span>
                      <span className="flex items-center gap-2">
                        <Calendar size={14} />
                        {new Date(featuredPost.date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </span>
                    </div>
            ...
            >
              {otherPosts.length > 0 ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-warm-500 text-lg">No articles found matching your search.</p>
                  <button 
    ...
    >
      <Link href={`/blog/${post.slug}`} className="group block h-full">
        <article className="h-full flex flex-col bg-white rounded-3xl border border-warm-200/60 overflow-hidden hover:border-warm-300 hover:shadow-xl transition-all duration-500">
          <div className="aspect-[16/10] bg-gradient-to-br from-primary-50 via-warm-50 to-sage-50 relative overflow-hidden">
            <div className="absolute inset-0 flex items-center justify-center text-4xl opacity-40">📄</div>
            <div className="absolute top-4 left-4">
              <span className="px-3 py-1 rounded-full bg-white/90 backdrop-blur-sm text-warm-700 text-xs font-medium shadow-sm">
                {post.category}
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 p-6 lg:p-8 flex flex-col">
            <h3 className="text-xl font-display font-semibold text-warm-900 mb-3 group-hover:text-primary-700 transition-colors leading-snug">
              {post.title}
            </h3>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-warm-100">
              <div className="flex items-center gap-4 text-xs text-warm-400">
                <span className="flex items-center gap-1.5">
                  <User size={12} />
                  {(post.author || 'Aliento').replace('Aliento ', '').replace('Dr. ', '')}
                </span>
                <span className="flex items-center gap-1.5">
                  <Calendar size={12} />
              ...
              <span className="text-primary-500 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                <ArrowRight size={16} />
              </span>
            </div>
          </div>
